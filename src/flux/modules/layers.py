...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, info) -> Tensor:
        ...
        feature_injection_mode = info.get('feature_injection_mode')
        in_feature_range = info['id'] <= info['end_layer_index'] and info['id'] >= info['start_layer_index']
        if feature_injection_mode is None:
            save_features = bool(info['inject'] and info['inverse'])
            load_features = bool(info['inject'] and not info['inverse'])
        elif feature_injection_mode == 'save':
            save_features = True
            load_features = False
        elif feature_injection_mode == 'load':
            save_features = False
            load_features = True
        elif feature_injection_mode == 'off':
            save_features = False
            load_features = False
        else:
            raise ValueError(f"Unsupported feature injection mode: {feature_injection_mode}")

        if in_feature_range and (save_features or load_features):
